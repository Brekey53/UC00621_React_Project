...
router.put("/:id", authenticateToken, (req, res) => {
  ...
  const sql =
    "UPDATE tasks SET title = ?, description = ?, status = ?, date = ? WHERE id = ? AND user_id = ?";
  ...
  db.run(sql, [title, description, status, date, id, userId], function (err) {
    ...
    if (this.changes === 0) {
      res.status(404).json({
        message: "Task was not found or user dont have access to it",
      });
      return;
    }
    ...
    function (err) {
      ...
      if (this.changes === 0) {
        return res.status(404).json({
          message:
            "Task was not found or that the user does not have permission to delete it.",
        });
      }

      res
        .status(200)
        .json({ message: `Task with ID ${id} was sucessfully deleted.` });
    }
  );
